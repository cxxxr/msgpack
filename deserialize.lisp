...
(defun octets-to-utf8-string (octets length pos)
  (babel:octets-to-string octets
                          :start pos :end (+ pos length)
                          :encoding :utf-8))

(defun deserialize (octets &optional (pos 0))
  (labels ((decode-unsigned-integer (n)
             (prog1 (load-big-endian octets n pos)
               (incf pos n)))
           (decode-signed-integer (n)
             (prog1 (octets-to-signed-integer octets n pos)
               (incf pos n)))
           (decode-ieee-float (nbits n)
             (let ((bits (load-big-endian octets n pos)))
               (prog1 (ecase nbits
                        (32 (ieee-floats:decode-float32 bits))
                        (64 (ieee-floats:decode-float64 bits)))
                 (incf pos n))))
           (decode-utf8-string (length n)
             (prog1 (octets-to-utf8-string octets length (+ pos n))
               (incf pos (+ n length))))
           (decode-utf8-string* (n)
             (decode-utf8-string (load-big-endian octets n pos) n))
           (decode-byte-array (n)
    ...
    (let ((octet (aref octets pos)))
      ...
      (values (cond ((= octet #xC0) nil)
                    ...
                    ((= #b101 (ldb (byte 3 5) octet))
                     (decode-utf8-string (ldb (byte 5 0) octet) 0))
                    ((= octet #xD9) (decode-utf8-string* 1))
                    ((= octet #xDA) (decode-utf8-string* 2))
                    ((= octet #xDB) (decode-utf8-string* 4))
